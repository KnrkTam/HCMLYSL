...
class Bk_unit_eva extends CI_Controller //change this
{
    private $scope = 'unit_eva'; //change this
    ...
    public function print()
    {
        ...
        $this->load->view('webadmin/' . $this->scope . '_print',  $data);
    }


    public function printfirm()
    {
        $data['page_setting'] = $this->page_setting(array(
            'view_news'
        ), FALSE, TRUE);
        ...
        $data['action'] = __('確認打印項目');

        $GLOBALS["select2"] = 1;

        $GLOBALS["datatable"] = 1;
        $this->load->view('webadmin/' . $this->scope . '_printfirm',  $data);
    }
    public function printpreview()
    {

        $this->load->view('webadmin/' . $this->scope . '_printpreview',);
    }
