...
class Bk_annual_subject_group extends CI_Controller //change this
{
    ...
    public function ajax(){
        ...
        if (!empty( $result)) {
            foreach ($result as $key => $row) {
                ...
                $data[$num]['other_staff'] = $other_staff ? $other_staff : '<p style="color:lightgrey">暫無其他任教</p>';
    ...
    public function edit($id)
    {
        ...
        $student_result = Annual_subject_groups_students_model::where('annual_subject_group_id', $id)->pluck('student_id')->toArray();
    ...
    public function submit_form($id = null){
        ...
        foreach ($module_order as $row) {
            ...
            $annual_subject_group_id = Annual_subject_groups_model::updateOrCreate($data, $add_content)->id;
            if (!$id) {
            // Create
                foreach ($student_id as $student) {
                    $student_data = array(
                        'annual_subject_group_id' => $annual_subject_group_id,
                        'student_id' => $student,
                    );
                    Annual_subject_groups_students_model::create($student_data);
                }
                ...
                foreach ($other_staff_id as $staff) {
                    ...
                    Annual_subject_groups_other_staff_model::create($other_staff_data);
                }
            } else if ($id) {
                // Edit
                $new_staff_arr = $other_staff_id;
                $old_staff_arr = Annual_subject_groups_other_staff_model::where('annual_subject_group_id', $id)->pluck('staff_id')->toArray();
                sort($new_staff_arr);
                sort($old_staff_arr);
                ...
                foreach ($delete_staff as $row) {
                    Annual_subject_groups_other_staff_model::where('annual_subject_group_id', $id)->where('staff_id', $row)->update(array('status' =>  0, 'deleted' => 1));
                };
                foreach ($add_staff as $row) {
                    ...
                    Annual_subject_groups_other_staff_model::updateOrCreate($staff_data, array('status' => 1, 'deleted' => 0));
                };

                // dump($student_id);
                $new_student_arr = $student_id;
                $old_student_arr = Annual_subject_groups_students_model::where('annual_subject_group_id', $id)->pluck('student_id')->toArray();
                ...
                foreach ($delete_student as $row) {
                    Annual_subject_groups_students_model::where('annual_subject_group_id', $id)->where('student_id', $row)->update(array('status' =>  0, 'deleted' => 1));
                };
                foreach ($add_student as $row) {
                    ...
                    Annual_subject_groups_students_model::updateOrCreate($student_data, array('status' => 1, 'deleted' => 0));
                };
            }
        }
