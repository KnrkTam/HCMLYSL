...
class Subject_lessons_model extends BaseModel
{
    ...
    public static function id_list($subject_id, $subject_category_id = null)
    {        
        $result = Subject_lessons_model::where('subject_id', $subject_id)->get();
        if ($subject_category_id) {
            $result = Subject_lessons_model::where('subject_id', $subject_id)->where('subject_category_id', $subject_category_id)->get();
        }
        foreach ($result as $i => $row){
